...
@implementation LJCacheManager

+ (LJCacheManager *)shareManager {
    static LJCacheManager *_LJCacheManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _LJCacheManager = [[LJCacheManager alloc] init];
    });
    return _LJCacheManager;
}
...
- (void)cacheResponseObject:(id)responseObject
    ...
    if (error == nil) {
        //缓存到内存中
        [LJMemoryCache writeData:responseObject forKey:hash];
        ...
        if (!directoryPath) {
            directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"LJNetworking"] stringByAppendingPathComponent:@"networkCache"];
            [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:cacheDirKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        [LJDiskCache writeData:data toDir:directoryPath filename:hash];
        
        [[LJLRUManager shareManager] addFileNode:hash];
    }
...
- (id)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
    ...
    //先从内存中查找
    cacheData = [LJMemoryCache readDataWithKey:hash];
    
    if (!cacheData) {
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        
        if (directoryPath) {
            cacheData = [LJDiskCache readDataFromDir:directoryPath filename:hash];
            
            if (cacheData) [[LJLRUManager shareManager] refreshIndexOfFileNode:hash];
        }
    }
...
- (void)storeDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"LJNetworking"] stringByAppendingPathComponent:@"download"];
    ...
    [LJDiskCache writeData:data toDir:directoryPath filename:fileName];
...
- (NSURL *)getDownloadDataFromCacheWithRequestUrl:(NSString *)requestUrl {
    ...
    if (directoryPath) data = [LJDiskCache readDataFromDir:directoryPath filename:fileName];
...
- (NSUInteger)totalCacheSize {
    ...
    return [LJDiskCache dataSizeInDir:diretoryPath];
}
...
- (void)clearDownloadData {
    ...
    [LJDiskCache clearDataIinDir:diretoryPath];
}
...
- (void)clearTotalCache {
    ...
    [LJDiskCache clearDataIinDir:directoryPath];
}

- (void)clearLRUCache {
    if ([self totalCacheSize] > diskCapacity) {
        NSArray *deleteFiles = [[LJLRUManager shareManager] removeLRUFileNodeWithCacheTime:cacheTime];
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath && deleteFiles.count > 0) {
            [deleteFiles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [LJDiskCache deleteCache:filePath];
            }];
